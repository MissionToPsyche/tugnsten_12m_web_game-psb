...
public class MagnetometerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        List<(Vector3, Vector3)> fieldPoints = arrowGenerator.getFieldPoints(torus, numPoints, numArrows);
    ...
    //     for (int i = 0; i < torus.torusObject.transform.childCount; i++)
    //     {
    //         GameObject child = torus.torusObject.transform.GetChild(i).gameObject;
    //         ellipses.Add(child);
    //     }
    ...
    //         } while (ellipseNum == Mathf.Floor(torus.torusObject.transform.childCount / 2));
